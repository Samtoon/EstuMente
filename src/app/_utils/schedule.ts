...
export function getAvailableHours(date: Date, appointments: IUpcomingAppointment[], schedule: ISchedule, test?: boolean) {
    const hours: boolean[] = [];
...
export function isSameDay(date1: Date, date2: Date) {
    return isEqual(new Date(date1).setHours(0, 0, 0, 0), new Date(date2).setHours(0, 0, 0, 0));
}
