...
import { createRequest, getRequestsByUser, getRequestsByUserRole, updateRequest } from "@/app/_database/daos/requestDao";
...
import { updateUserByEmail, updateUserById } from "@/app/_database/daos/userDao";
...
export async function hasPendingRequests(userId: string) {
    ...
    return Boolean(requests.filter((request) => request.state === RequestStates.Pendiente).length)
    
}

export async function sendRequest(formData: FormData, user: IUser, requestedRole: Roles) {
...
export async function answerRequest(request: IRequest, state: RequestStates, requestingUser: IUser, comment?: string) {
    ...
    if (state === RequestStates.Aprobado) {
        switch (request.requestedRole) {
            case Roles.Practicante:
                const fullName = requestingUser.firstName + " " + requestingUser.lastName;
            ...
            default:
                await updateUserById(request.user, {role: request.requestedRole, supportingDocumentId: request.supportingDocumentId});
        }
    }
    sendNotification(
