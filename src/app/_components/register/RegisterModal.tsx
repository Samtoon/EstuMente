...
export default function RegisterModal({
    ...
    function handleSubmit(formData: FormData) {
        if (user) {
            sendRequest(formData, user, selectedRole)
            .then((request) => {
                [Roles.Tutor, Roles.Coordinador, Roles.Administrador].forEach((role) => {
                    sendNotification(
    ...
    return (
        ...
        >
            ...
            <Stack>
                <RoleCardList selectedRole={selectedRole} setSelectedRole={setSelectedRole}/>
                {uploadedFile && <div>
                        ...
                        <form id="pruebaForm" action={handleSubmit}>
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setFileURL(e.target.files?.[0] ? URL.createObjectURL(e.target.files?.[0]) : "");
                                }}
                                accept=".pdf"
                            />
                        </form>
