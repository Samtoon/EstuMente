...
import { Avatar, Button, Dialog, DialogTitle, Divider, Grid, List, Skeleton, TextField } from "@mui/material";
...
export default function RequestModal({ open, handleClose, requestList, index }: Props) {
    ...
    function handleClick(state: RequestStates) {
        answerRequest(requestList[index], state, user!, comment)
        .then(() => {
    ...
    return (
        <Dialog open={open} onClose={handleClose}>
            ...
            <Grid container px={3} spacing={1} columns={12} pb={3}>
                ...
                <RequestField field="Rol solicitado:" value={requestList[index]?.requestedRole} />
                <RequestField field="Fecha solicitud:" value={requestList[index]?.createdAt?.toString()} />
                <RequestField field="Documento Soporte:" value={
                    <Link target="_blank" href={documentSrc}>
                        <Button color="secondary">Ver</Button>
                    </Link>
                } skeleton={<Button color="secondary" disabled>Ver</Button>} />
                <Grid item xs={12}>
                    <Divider id="divisor" />
                </Grid>
                <Grid item xs={12}>
