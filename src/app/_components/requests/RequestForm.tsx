...
function RequestField({ field, value }: { field: string, value?: string | JSX.Element }) {
    return (
        <>
            <Grid item xs={5}>
                {field}
...
export default async function RequestForm({ request }:{request: IRequest}) {
    ...
    const documentSrc = await fetch(process.env.NEXT_PUBLIC_BASE_URL + FILES + "/" + request.supportingDocumentId)
    ...
    return(
        <>
                ...
                <RequestField field="Fecha solicitud:" value={request.createdAt?.toString()}/>
                <RequestField field="Documento Soporte:" value={
                    ...
                    :
                    <Button color="secondary" disabled>Ver</Button>
                }/>
        </>
    );
}